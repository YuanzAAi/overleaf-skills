...
需要 **Python 3.10+** 和 **Git**，且命令行能够找到它们。选择对应的 coding agent 安装，完成后重新打开会话。
...
Codex 和 Claude Code 共用脚本和操作说明。安装器为 Codex 安装 `agents/openai.yaml`，仅在 Claude Code 的入口中加入 `allowed-tools` 字段。
...
仅使用自己有权访问的凭据和项目。Cookie 用于项目管理、协作、读取和下载源码、编译；文件内容编辑与 Git 历史需要账号已具有 Overleaf Git 集成权限。重新设置一组凭据即可换号。
...
支持项目设置、批量文件操作、ZIP 导入、归档与回收站、共享与协作审阅，并可复用编译产物。

完整命令见 [SKILL.md](SKILL.md)，或运行 `python <安装目录>/scripts/overleaf_skill.py --help`，macOS 使用 `python3`。

## 本地状态

每个 coding agent 使用各自的 `<agent-home>/overleaf-skills/state.json`，首次设置凭据时创建，包含**明文凭据**，请妥善保管。仓库不包含任何账号信息，`account show` 只显示脱敏值。凭据优先级为命令参数、`OVERLEAF_SESSION` / `OVERLEAF_GIT_TOKEN`、已保存状态。
